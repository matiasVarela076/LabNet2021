...
namespace Ejercicio4.EF.Logic
{
    ...
    {                                                                           //Que implemente los metodo Tipo Territories
        ...
        //METODO PARA RESFRECAR LA LISTA (consejo tomado de "mati malo" asi que si esta mal agarrense con el)XD
        public void RefrescarLista(TerritoriosLogica nuevoCampo)
        {
            foreach (Territories listaTerritorios in nuevoCampo.GetAll())
            {
                Console.WriteLine($"{listaTerritorios.TerritoryID} - {listaTerritorios.TerritoryDescription}");
            }
            Console.ReadKey();
